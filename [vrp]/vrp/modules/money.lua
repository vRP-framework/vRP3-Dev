...
function Money:__construct()
  ...
  -- copy only necessary config fields and free the cfg to reduce retained memory
  self.open_wallet = self.cfg.open_wallet
  self.open_bank = self.cfg.open_bank
  self.money_display = self.cfg.money_display
  self.display_css = self.cfg.display_css
  self.money_type = self.cfg.money_type
  self.currency_symbols = self.cfg.currency_symbols
  self.money_delta = self.cfg.money_delta
  self.delta_colors = self.cfg.delta_colors
  self.lose_wallet_on_death = self.cfg.lose_wallet_on_death
  self.cfg = nil
...
function Money:formatWallet(amount)
  local type = self.money_type or "USD"
  local symbol = (self.currency_symbols or {})[type] or ""
...
function Money.event:characterLoad(user)  -- init character money
  if not user.cdata.wallet then
    user.cdata.wallet = self.open_wallet
  end

  if not user.cdata.bank then
    user.cdata.bank = self.open_bank
  end
...
function Money.event:playerSpawn(user, first_spawn)
  if self.money_display and first_spawn then
    vRP.EXT.GUI.remote._setDiv(user.source, "money", self.display_css, self:formatWallet(user:getWallet()))
  end
end

function Money.event:playerDeath(user)
  if self.lose_wallet_on_death then
    user:setWallet(0)
  end
end

function Money.event:playerMoneyUpdate(user)
  if not self.money_display then return end
  ...
  if self.money_delta then
    ...
    if delta ~= 0 then
      ...
      local color = (sign == "+") and (self.delta_colors and self.delta_colors.positive) or (self.delta_colors and self.delta_colors.negative)
      
      vRP.EXT.GUI.remote._setDiv(user.source, "delta", self.display_css, {sign .. delta})
      vRP.EXT.GUI.remote._setDivCss(user.source, "delta", ".div_delta{ color: ".. (color or "#fff") .."; }")

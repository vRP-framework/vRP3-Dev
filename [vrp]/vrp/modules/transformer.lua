...
-- localize globals for hot loops
local pairs = pairs
local ipairs = ipairs
local next = next
...
function TransformerDef:unbindUser(user)
  ...
  if recipe_name then
    self.users[user] = nil
    if self._user_count and self._user_count > 0 then self._user_count = self._user_count - 1 end
  ...
  -- if no more users, mark inactive on parent extension
  if self.ext and self.ext.active_transformers and next(self.users) == nil then
    self.ext.active_transformers[self.id] = nil
  end
end
...
function TransformerDef:bindUser(user, recipe_name)
  ...
  self._user_count = (self._user_count or 0) + 1
  ...
  -- register active transformer on parent extension
  if self.ext and self.ext.active_transformers then
    self.ext.active_transformers[self.id] = self
  end
end
...
function TransformerDef:tick()
  -- skip work if no users bound
  if next(self.users) == nil then return end

  local processors = vRP.EXT.Transformer.processors
  local t0 = GetGameTimer()
  local user_count = self._user_count or 0
  ...
  -- display transformation state to all transforming players (throttled)
...
function Transformer:__construct()
  ...
  self.active_transformers = {} -- map of id => transformer with users
